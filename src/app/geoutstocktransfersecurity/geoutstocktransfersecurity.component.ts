...
declare var $: any;
@Component({
  selector: 'app-geoutstocktransfersecurity',
  templateUrl: './geoutstocktransfersecurity.component.html',
  styleUrls: ['./geoutstocktransfersecurity.component.css']
})
      ...
      this.httpService.post(APIURLS.BR_MASTER_GATEOUTWARDM_FILTER_API, genericGateEntryM).then((data: any) => {
        if (data) {
          ...
          let goData = data.filter((item: any) => {
            return (item.gO_TYPE == "4" && item.isActive);
          });
          if (goData.length > 0) {

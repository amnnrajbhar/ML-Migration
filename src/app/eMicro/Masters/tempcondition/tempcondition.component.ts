...
import {TempCondition} from '../tempcondition/tempcondition.model';
...
@Component({
  selector: 'app-tempcondition',
  templateUrl: './tempcondition.component.html',
  styleUrls: ['./tempcondition.component.css']
})
export class TempConditionComponent implements OnInit {
  ...
  TempCondition: TempCondition = new TempCondition();
  ...
  oldTempCondition: TempCondition = new TempCondition();// For aduit log
  ...
  onAddTempCond(isEdit: boolean, data: TempCondition) {
    ...
    this.aduitpurpose='';
    this.oldTempCondition=new TempCondition();
    if (this.isEdit) {
      // this.materialItem = data;
      Object.assign(this.oldTempCondition, data);
      Object.assign(this.TempCondition, data);
    }
    else {
      this.TempCondition = new TempCondition();
    }
  ...
  getTempConditionList() {
    this.httpService.get(APIURLS.BR_MASTER_TEMP_COND_ALL_API).then((data: any) => {
      this.isLoading = true;
      if (data.length > 0) {
        this.tempconditionlist = data;
      }
      this.reInitDatatable();
      this.isLoading = false;
    }).catch(error => {
  ...
  onSaveTempCond() {
    ...
    let connection: any;
      ...
      if (!this.isEdit)
      {
        ...
        this.TempCondition.createdBy=this.currentUser.employeeId;;
        this.TempCondition.createdOn=new Date().toLocaleString();
        connection = this.httpService.post(APIURLS.BR_MASTER_TEMP_COND_POST_PUT_API, this.TempCondition);
      }
       
      else
      {
        this.auditType="Update";
        this.TempCondition.lastModifiedBy=this.currentUser.employeeId;;
        this.TempCondition.lastModifiedOn=new Date().toLocaleString();
        connection = this.httpService.put(APIURLS.BR_MASTER_TEMP_COND_POST_PUT_API, +this.TempCondition.tempConId, this.TempCondition);
      }
      connection.then((data: any) => {
        this.isLoadingPop = false;
        if (data == 200 || data.tempConId > 0) {
          jQuery("#myModal").modal('hide');
          this.errMsgPop1 = 'Temparature Condition saved successfully!';
          jQuery("#saveModal").modal('show');
          let Id=!this.isEdit?data.tempConId:this.TempCondition.tempConId;
          this.insertAuditLog(this.oldTempCondition,this.TempCondition,Id);
          this.getTempConditionList();
        }
      }).catch(error => {
        this.isLoadingPop = false;
  ...
  closeSaveModal() {
   ...
   //AuditLogging
   masterName:string='Temparature Condition'; // Change MasterName
   insertAuditLog(oldObj: TempCondition, newObj: TempCondition, id) {
     let oldObject: actionItemModel = new actionItemModel();
     let newObject: actionItemModel = new actionItemModel(); 
    
     oldObject.tempConDesc = oldObj.tempConDesc;
     oldObject.isActive=oldObj.isActive;
 
    
     newObject.tempConDesc = newObj.tempConDesc;
     ...
     auditlog.tableId = id;
     auditlog.keyValue=newObj.tempConDesc?newObj.tempConDesc:oldObj.tempConDesc;

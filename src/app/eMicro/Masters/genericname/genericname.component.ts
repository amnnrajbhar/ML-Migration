...
import {GenericName} from '../genericname/genericname.model';
...
@Component({
  ...
  templateUrl: './genericname.component.html',
  styleUrls: ['./genericname.component.css']
})
...
@ViewChild(NgForm, { static: false }) materialForm!: NgForm;
  ...
  oldGenericName: GenericName = new GenericName();// For aduit log
...
this.currentUser = storedUser ? JSON.parse(storedUser) : null;
  ...
  onAddGenericName(isEdit: boolean, data: GenericName) {
    ...
    this.aduitpurpose='';
    this.oldGenericName=new GenericName();
    if (this.isEdit) {
      // this.materialItem = data;
      Object.assign(this.oldGenericName, data);
      Object.assign(this.GenericName, data);
    }
    else {
  ...
  getGenericNameList() {
    this.httpService.get(APIURLS.BR_MASTER_GENERIC_NAME_ALL_API).then((data: any) => {
      this.isLoading = true;
      if (data.length > 0) {
  ...
  onSaveGenericName() {
    ...
    let connection: any;
      ...
      if (!this.isEdit)
      {
        ...
        this.GenericName.createdBy=this.currentUser.employeeId;;
        this.GenericName.createdOn=new Date().toLocaleString();
        connection = this.httpService.post(APIURLS.BR_MASTER_GENERIC_NAME_POST_PUT_API, this.GenericName);
      }
       
      else
      {
        this.auditType="Update";
        this.GenericName.lastModifiedBy=this.currentUser.employeeId;;
        this.GenericName.lastModifiedOn=new Date().toLocaleString();
        connection = this.httpService.put(APIURLS.BR_MASTER_GENERIC_NAME_POST_PUT_API, this.GenericName.id, this.GenericName);
      }
      connection.then((data: any) => {
        this.isLoadingPop = false;
        if (data == 200 || data.id > 0) {
          jQuery("#myModal").modal('hide');
          this.errMsgPop1 = 'Generic Name saved successfully!';
          jQuery("#saveModal").modal('show');
          let Id=!this.isEdit?data.id:this.GenericName.id;
          this.insertAuditLog(this.oldGenericName,this.GenericName,Id);
          this.getGenericNameList();
        }
      }).catch((error)=> {
        this.isLoadingPop = false;
        this.errMsgPop = 'Error saving Generic Name..';
      });
  ...
  closeSaveModal() {
   ...
   //AuditLogging
   masterName:string='Generic Name'; // Change MasterName
   insertAuditLog(oldObj: GenericName, newObj: GenericName, id) {
     let oldObject: actionItemModel = new actionItemModel();
     let newObject: actionItemModel = new actionItemModel(); 
    
     oldObject.genNameDesc = oldObj.genNameDesc;
     oldObject.isActive=oldObj.isActive;
 
    
     newObject.genNameDesc = newObj.genNameDesc;
     ...
     auditlog.tableId = id;
     auditlog.keyValue=newObj.genNameDesc?newObj.genNameDesc:oldObj.genNameDesc;

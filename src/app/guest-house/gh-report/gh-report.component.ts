...
@Component({
  selector: 'app-gh-report',
  templateUrl: './gh-report.component.html',
...
export class GhReportComponent implements OnInit {
  ...
  getMyMeetings() {
    ...
    genericBookingFilters.empLocation="Report";
    if (this.selectedLocations != null) {
      let locations = null;
      this.selectedLocations.forEach(loc => {
        locations = loc.id + ',' + locations;
      });
      genericBookingFilters.roomLocation = locations;
    }
    ...
    if (this.currentUser.fkProfileId == 1004)
  ...
  getLocationList() {
    var genericBookingFilters = {} as GenericBookingFilters;
    if (this.currentUser.fkProfileId == 1004)
      genericBookingFilters.adminId = this.currentUser.uid.toString();
  ...
  onRoomTypeDeSelect(item: any) {
  }
  onRoomTypeDeSelectAll(items: any) {
    this.selectedRoomType = [];
  }
  onRoomTypeSelectAll(items: any) {
    this.selectedRoomType = items;
  }
  getAllroomsTypes() {
  ...
  exportAsXLSX(): void {
    ...
    this.myMeetings.forEach(item => {
      ...
      let exportItem={
        ...
        "Guest House Name":item.roomName,
        "Guest House Location":item.roomLocationName,
        "Purpose":item.purpose,
        "Start Time": this.setDateFormate(item.fromDate)+' '+item.fromTime,
        "End Time": this.setDateFormate(item.toDate)+' '+item.toTime,
        "Status": item.status,
    ...
    this.excelService.exportAsExcelFile(this.exportList, 'Guest House Booking Report');
  }

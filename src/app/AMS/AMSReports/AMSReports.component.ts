...
export class AMSReportsComponent implements OnInit {
  ...
  getSoftTypename(id) {
    let temp = this.softwareList.find(x => x.softId == id);
  ...
  getLicTypename(licId) {
    let temp = this.licenseList.find(x => x.licId == licId);
  ...
  getData() {
    ...
    filterModel.category = this.filtercategory.map(x => x).join();
    filterModel.usageType = this.filterusageType;
    filterModel.assetState = this.filterassetState.map(x => x.id).join();
  ...
  getData1() {
    ...
    filterModel.location = this.filterlocation.map(x => x.code).join();
    filterModel.assetType = this.filterassetType.map(x => x).join();
    filterModel.category = this.filtercategory.map(x => x).join();
  ...
  getData2() {
    ...
    filterModel.location = this.filterlocation.map(x => x.code).join();
    filterModel.softwareType = this.filtersoftwareType.map(x => x.softId).join();
    filterModel.licenseType = this.filterlicenseType.map(x => x.licId).join();
    filterModel.assetState = this.filterassetState.map(x => x.id).join();
    filterModel.pageNo = this.pageNo;
    filterModel.pageSize = this.pageSize;
    filterModel.FromDate = this.getFormatedDateTime(this.from_date);

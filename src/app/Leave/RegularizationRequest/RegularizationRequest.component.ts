...
import { RegularizationRequest } from './RegularizationRequest.model';
...
@Component({
  selector: 'app-RegularizationRequest',
  templateUrl: './RegularizationRequest.component.html',
  styleUrls: ['./RegularizationRequest.component.css']
})
export class RegularizationRequestComponent implements OnInit {
 ...
 private route: ActivatedRoute) { }
  ...
  getEmpPermissionRequests() {
    ...
    this.httpService.LApost(APIURLS.GET_EMPLOYEE_REG_REQUESTS, srchstr).then((data: any) => {
      if (data) {
  ...
  OnSaveRequest(status) {
    let filterModel = {} as RegularizationRequest;
    if (this.attendanceDetails[0].inStatus == 'PP' && this.SwipeType == 'In') {
      ...
      filterModel.reasonType = this.ReasonType;
      ...
      connection.then((data: any) => {
        ...
        if (data.type == 'S') {
          swal({
            title: "Message",
            text: "Regularization Request Details Submitted Sucessfully With Req Id : " + data.requestNo,
            icon: "success",
            dangerMode: false,
            buttons: [false, true]
          });
  ...
  Cancel(data) {
    swal({
      ...
      if (willsave) {
        ...
        let leaveDetailsModel = {} as RegularizationRequest;
        ...
        let connection = this.httpService.LAput(APIURLS.UPDATE_REGULARIZATION_REQUEST, leaveDetailsModel.id, leaveDetailsModel);
        connection.then((data: any) => {
          this.isLoading = false;
          if (data) {
            swal({
              title: "Message",
              text: "Regularization Request Cancelled Sucessfully..!",
              icon: "success",
              dangerMode: false,
              buttons: [false, true]
            });
            ...
            swal({
              title: "Message",
              text: "Error Cancelling Regularization Request ..",
              icon: "error",
              dangerMode: false,
              buttons: [false, true]
            });
          ...
          this.errMsgPop = 'Error Cancelling Regularization Request ..';
        });
      }
    });
  }

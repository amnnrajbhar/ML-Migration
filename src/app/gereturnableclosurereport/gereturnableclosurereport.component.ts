...
@Component({
  selector: 'app-gereturnableclosurereport',
  templateUrl: './gereturnableclosurereport.component.html',
  styleUrls: ['./gereturnableclosurereport.component.css']
})
export class GereturnableclosurereportComponent implements OnInit {
  ...
  loadGateOutwardList() {
    ...
    this.httpService.post(APIURLS.BR_GE_RETURN_CLOSURE_REPORT_API, genericGateEntryM).then((data: any) => {
      if (data) {
  ...
  exportAsXLSX(): void {
    ...
    this.gateOutwardMList.forEach(item => {
      ...
      let exportItem={
        ...
        "OUT TIME":item.ouT_TIME?this.setFormatedDateTime(item.ouT_TIME):'',
        "MATERIAL TYPE":item.materiaL_TYPE,
        "iteM DESC":item.iteM_DESC,
        "UOM":item.uom,
        "QTY":item.qty?item.qty:0,
        "QTY RCVD":item.qtY_RCVD!=null?item.qtY_RCVD:0,
        "STATUS":item.status=='1'?'Closed':'Pending'
      }
  ...
  downloadPdf()
  {
    ...
    var ReportName = "GE RETURNABLE CLOSURE REPORT"
  ...
  <div>     
    ...
    var docDefinition = {
      info: {
        title:'ReturnableClosure Report',
        },

...
import { RFCSTOHeader } from './rfcstoheader.model';

@Component({
  selector: 'app-geoutstocktransfer',
  templateUrl: './geoutstocktransfer.html',
...
this.currentUser = storedUser ? JSON.parse(storedUser) : null;
          ...
          if (data) {
            ...
            if (!existInvoice) {
              this.httpService.post(APIURLS.BR_RFCBAPI_GETGE_STO_SALES_INFO_API, geSTOInputs).then((data: any) => {
                if (data.length > 0) {
                  if (data[0].type == 'S') {
                    if (data[0].trntyp == 'STO') {
                      if (data[0].werks == this.plant) {
                        this.gateEntryHeaderModel = data[0];
                        this.selDestination = this.locationList.find((x:any)  => x.code == this.gateEntryHeaderModel.rcwrk);
                        ...
                        //this.gateEntryMaterial=this.gateEntryHeaderModel.gestoitems;
                      ...
                      swal({
                        title: "Message",
                        text: 'Entered invoice number is not for Stock transfer.Please enter correct invoice number',
                        icon: "warning",
                        dangerMode: false,
                        buttons: [false, true]
                      });
  ...
  insertGateOutstock() {
    ...
    this.gateOutwardMModel.destinatioN_NM = this.gateEntryHeaderModel.rcwrk;
  ...
  deactivateGateEntry() {
    ...
    this.gateOutwardMModel.deleteD_BY = this.userName;

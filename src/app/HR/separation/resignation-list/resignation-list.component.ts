...
import { ResignationListFilter } from './ResignationListFilter.model';
import { ResignationUpdateRequest } from './resignationupdaterequest.model';
...
this.currentUser = storedUser ? JSON.parse(storedUser) : null;
    ...
    this.filterModel.selectedShortFallDays = "";
    this.filterModel.pendingWith="";
    this.filterModel.createdByName="";
    this.filterModel.createdOn="";
    this.filterModel.resignationId='';
    // get filter model from the in memory data store
    var oldFilter = this.dataStore.GetData("ResignationList");
    if(oldFilter){
  ...
  sendExitInterviewEmail(id:any) {
    if (confirm("Are you sure you want to send exit interview email?")) {
      var request: any = {};
      request.resignationId = id;
      request.submittedById = this.currentUser.uid;
      request.submittedByName = this.currentUser.fullName;
      swal("Sending email...");
      this.httpService.HRpost(APIURLS.RESIGNATION_EXIT_INTERVIEW_LINK, request).then((data: any) => {
        if (data == 200 || data.success) {
          this.reisinationList1 = null;
          this.getData();
          swal("Successfully emailed the exit interview link to the employee.");

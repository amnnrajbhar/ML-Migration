...
@Component({
  selector: 'app-termination-checklist',
  templateUrl: './termination-checklist.component.html',
  styleUrls: ['./termination-checklist.component.css'],
  providers: [Util]
})
...
this.currentUser = storedUser ? JSON.parse(storedUser) : null;
  ...
  SaveData(){
    ...
    this.isLoading = true;
    let data: any = {};
    data.terminationId = this.terminationId;
    data.list = this.checklistItemList;
    data.updatedById = this.currentUser.uid;    
    connection = this.httpService.HRpost(APIURLS.TERMINATION_SAVE_CHECKLIST_ITEMS, data);
  ...
  GetChecklistItems() {   
    ...
    if(this.terminationId && this.terminationId > 0){
      this.isLoading = true;
      this.httpService.HRget(APIURLS.TERMINATION_GET_CHECKLIST_ITEMS+"/"+ this.terminationId).then((data: any) => {
        if (data) {

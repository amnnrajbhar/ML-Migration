...
@Component({
  selector: 'app-confirmation-employee-readonlysalary',
  templateUrl: './confirmation-employee-readonlysalary.component.html',
  styleUrls: ['./confirmation-employee-readonlysalary.component.css']
})
export class ConfirmationEmployeeReadonlysalaryComponent implements OnInit {
  ...
  headTypes = [{type:"I", value:"Income"}, {type:"D", value:"Deduction"}, {type:"R", value:"Reimbursement"}, {type:"B", value:"Other Benefit"}, {type:"V", value:"Variable Pay"}, {type:"O", value:"One Time"}];
  ...
  LoadData() {
    ...
    this.httpService.HRget(APIURLS.HR_EMPLOYEE_GET_SALARY_DETAILS + "/" + this.employeeId ).then((data: any) => {
      if (data) {
        this.details = data;
        this.allowanceDetails =  data.alowanceText;

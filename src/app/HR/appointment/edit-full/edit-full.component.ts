...
@Component({
  selector: 'app-edit-full',
  templateUrl: './edit-full.component.html',
  styleUrls: ['./edit-full.component.css'],
  providers: [Util]
})
...
onNext(){
  if(this.currentTab == "initial"){
    if(this.officialDetails.validate())
...
onDataSaved(result){
  if(result == 200 || result.success || result.id > 0){
    if(this.currentTab == "initial" && !this.appointmentCreated){

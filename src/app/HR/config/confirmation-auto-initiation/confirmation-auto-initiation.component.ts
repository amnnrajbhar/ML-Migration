...
@Component({
  selector: 'app-confirmation-auto-initiation',
  templateUrl: './confirmation-auto-initiation.component.html',
  styleUrls: ['./confirmation-auto-initiation.component.css'],
  providers: [Util]
})
export class ConfirmationAutoInitiationComponent implements OnInit {
  ...
  editIndex: number = -1;
  initiateThruList = ["HR", "HOD", "RM & HOD", "Config"];
  plantList: any[] = [];
  payGroupList: any[] = [];
  ...
  addData(){
    ...
    this.httpService.HRpost(APIURLS.HR_CONFIRMATION_AUTO_INITIATION_CONFIG_API, this.item)
    .then((data: any) => {
      if (data == 200 || data.confirmationAutoInitiationConfigId > 0) {
        this.item.confirmationAutoInitiationConfigId = data.confirmationAutoInitiationConfigId;
        this.onAddLine();
  ...
  updateData(){
    ...
    this.httpService.HRput(APIURLS.HR_CONFIRMATION_AUTO_INITIATION_CONFIG_API, this.item.confirmationAutoInitiationConfigId, this.item)
    .then((data: any) => {
  ...
  deleteData(id, no){
    ...
    let connection: any;
    let data: any = {};
    data.confirmationAutoInitiationConfigId = id;
    toastr.info('Deleting...');
    this.isLoading = true;
    this.httpService.HRdelete(APIURLS.HR_CONFIRMATION_AUTO_INITIATION_CONFIG_API, id)
    .then(
      (data: any) => {
        this.isLoading = false;       

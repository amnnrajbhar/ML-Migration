...
@Component({
  selector: 'app-offer-email-notification',
  templateUrl: './offer-email-notification.component.html',
  styleUrls: ['./offer-email-notification.component.css'],
  providers:[Util]
})
export class OfferEmailNotificationComponent implements OnInit {
  ...
  notificationEmails = new FormGroup({
    offerEmail: new FormControl('',[
      Validators.required,
  ...
  validate(action:any)
  {
    ...
    if(this.emailNotificationDetails.emailId == "")
    {
  ...
  performaction(action:any){
    ...
    {
      this.isLoading = true;
    this.emailNotificationDetails.plantId = this.selectedPlant;
    this.emailNotificationDetails.payGroupId=this.selectedPayGroup.id;
    this.emailNotificationDetails.employeeCategoryId=this.selectedEmployeeCategory.id;
    if ( this.selectedState != null)
    {this.emailNotificationDetails.stateId = this.selectedState.id};
    this.emailNotificationDetails.eMailType = this.selectedEmailType.type;
    this.emailNotificationDetails.createdById = this.currentUser.uid;

    if(action=="Save")
    {
      connection = this.httpService.HRpost(APIURLS.EMAIL_NOTIFICATION_CREATE, this.emailNotificationDetails);
    }
    connection.then(
      (data: any) => {

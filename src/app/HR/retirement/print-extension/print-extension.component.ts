...
@Component({
  ...
  styleUrls: ['./print-extension.component.css'],
  providers: [Util]
})
export class PrintExtensionComponent implements OnInit {
  ...
  LoadretirementDetails() {
    ...
    this.httpService.HRgetById(APIURLS.RETIREMENT_GET_DETAILS_BY_ID, this.retirementId).then((data: any) => {
      if (data) {
  ...
  getLetter() {
    ...
    this.httpService.HRget(APIURLS.RETIREMENT_GET_LETTER_FOR_PRINT+"/"+this.retirementId
    +"/"+this.letterType).then((data: any) => {
      if (data) {
        this.details = data;        
        this.LoadretirementDetails();
  ...
  sendEmail() {
    this.validateEmail(this.retirementDetails.personalEmailId);
    if (this.isEmailValid == false)
    {
      toastr.error("Please enter valid email Id.");
      return;
    }
    if(this.retirementDetails.personalEmailId == undefined || this.retirementDetails.personalEmailId ==''){
      toastr.error("Please enter email Id.");
      return;
    }
    if (confirm("Are you sure you want to send email?")) {
      var request: any = {};
      request.retirementId = this.retirementDetails.retirementId;
      request.submittedById = this.currentUser.uid;
      request.submittedByName = this.currentUser.fullName;
      request.emailId = this.retirementDetails.personalEmailId;
      request.letterType = this.details.letterType;

...
@Component({
  selector: 'app-pending-extension-list',
  templateUrl: './pending-extension-list.component.html',
  styleUrls: ['./pending-extension-list.component.css']
})
export class PendingExtensionListComponent implements OnInit {
  ...
  taskId: number = 0;
  comments: string
  types = [{type:"Retirement Approval"}];
  plantList: any[] = [];
  payGroupList: any[] = [];
  empCategoryList: any[] = [];
  errorCount =0;
...
getEmployeeCategoryList() {
  ...
  LoadMyPendingTasks() {
    ...
    this.httpService.HRget(APIURLS.RETIREMENT_PENDING_TASKS+"?userId="+ userId
  ...
  approveBulk(){
    ...
    {      
      var request = {} as CompleteRetirementTaskRequest;
      ...
      this.httpService.HRpost(APIURLS.RETIREMENT_APPROVE_TASK, request).then((data: any) => {
        if (data) {
          if(!data.success){
            ...
            swal("Retirement Approved successfully.");
            this.myTasks = null;
            this.LoadMyPendingTasks();
          }
        }
    ...
    this.httpService.HRpost(APIURLS.RETIREMENT_REJECT_TASK, request).then((data: any) => {
      if (data) {
        if(!data.success){
          ...
          swal("Retirement Rejected successfully.");
          this.myTasks = null;
          this.LoadMyPendingTasks();
        }
      }
    ...
    this.isLoading = true;    
      ...
      this.myTasks.forEach((item :any) => {
        ...
        let exportItem={
          ...
          "Tenure":item.tenure,        
          "Age":item.age,     
          "Retirement Date":item.dateOfRetirement,   
          "Extension Start Date":item.extensionStartdate,                             
          "Months":item.months,                          
          "Extension End Date":item.extensionEnddate,
          "Remarks":item.remarks,

        };
          exportList.push(exportItem);
      });
      this.excelService.exportAsExcelFile(exportList, 'Pending_RetirementExtensions'); 
      this.isLoading = false;

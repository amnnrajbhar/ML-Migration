...
this.currentUser = storedUser ? JSON.parse(storedUser) : null;
    ...
    this.filterModel.checklistType = "Employee-Exit Checklist";
    this.filterModel.objectType = "";
    this.filterModel.status="";
    this.getPlantList();
    this.getEmployeeCategoryList();
    
      // get filter model from the in memory data store
      var oldFilter = this.dataStore.GetData("SeparationChecklist");
      if(oldFilter){
  ...
  getData(){
    this.isLoading = true;    
    this.httpService.HRpost(APIURLS.CHECKLIST_GET_CHECKLIST_ITEMS_BY_FILTER, this.filterModel).then((data: any) => {
      ...
      this.dataStore.SetData("SeparationChecklist", this.filterModel);

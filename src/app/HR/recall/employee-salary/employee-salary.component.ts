...
console.log("save data called");
  ...
  getCTCBreakup(){
   ...
   if(this.details.revisedSalaryType == "addtionalGross"){
    ...
    this.httpService.HRget(APIURLS.HR_EMPLOYEE_GET_CTC_BREAKUP + "/" + this.employeeId + "/"+this.details.revisedAmount+"?catId="+this.newEmployeeCategoryId+"&dsgId="+this.newDesignationId).then((data: any) => {
      if (data) {
        this.salaryList = data.headDetails;
        var index=0;
        for(var item of this.salaryList){

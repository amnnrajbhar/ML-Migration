...
import { ContractEmployee } from './ContractEmployeeList.model';
...
@Component({
  selector: 'app-ContractEmployeeList',
  templateUrl: './ContractEmployeeList.component.html',
...
//    filterModel.isActive = this.filterStatus='Active'?1:this.filterStatus='In Active'?0:this.filterStatus='ToBe Approved'?2:this.filterStatus='Blacklisted'?3:'';
  ...
  saveEmployee() {
    ...
    connection = this.httpService.LApost(APIURLS.CONTRACT_EMPLOYEE_INSERT, this.userMasterItem);

    connection.then((dataaddress: any) => {
      if (dataaddress.id > 0) {
        // jQuery("#myModal").modal('hide');
        alert(' Employee Created Successfully With Employee Id ' + dataaddress.employeeId);
        // jQuery("#saveModal").modal('show');
        this.clear();
      }
      this.isLoadingPop = false;
    ...
    swal({
      ...
      if(data)
      {
        ...
        connection.then((dataaddress: any) => {
          if (dataaddress == 200) {
            // jQuery("#myModal").modal('hide');
            alert('Employee Blacklisted Successfully');
            // jQuery("#saveModal").modal('show');
            this.getUsersList();
            this.clear();
          }
    ...
    swal({
      ...
      if(data)
      {
        ...
        connection.then((dataaddress: any) => {
          if (dataaddress == 200) {
            // jQuery("#myModal").modal('hide');
            alert('Employee Removed from Blacklist Succuessfully ');
            // jQuery("#saveModal").modal('show');
            this.getUsersList();
            this.clear();
          }
